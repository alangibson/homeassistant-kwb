...
from custom_components.kwb.sensor import PLATFORM_SCHEMA, setup_platform
...
class EntryTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_setup_stop_unload(self):
        ...
        client.run_thread.assert_called_once()
        self.hass.config_entries.async_forward_entry_setups.assert_awaited_once_with(
            self.entry, [Platform.SENSOR, Platform.BINARY_SENSOR]
        )
        ...
        self.assertEqual(client.stop_thread.call_count, 2)
    ...
    async def test_platform_failure_closes_client(self):
        ...
        with patch("custom_components.kwb.create_client", return_value=client):
            with self.assertRaises(RuntimeError):
                await async_setup_entry(self.hass, self.entry)
        client.stop_thread.assert_called_once()
    ...
    async def test_failed_unload_keeps_connection(self):
        ...
        self.entry.runtime_data.stop_thread.assert_not_called()
    ...
    async def test_real_pykwb_sensor_list(self):
        ...
        sensors = []
        await setup_sensors(self.hass, self.entry, sensors.extend)
        self.assertEqual(len(sensors), expected_count + 2)
        flags = []
    ...
    async def test_sensor_routing_and_flag_states(self):
        ...
        sensors = []
        with (
            patch("custom_components.kwb.sensor.kwb.KWBEasyfire", return_value=client),
            patch("custom_components.kwb.sensor.discovery.load_platform") as load,
        ):
            setup_platform(self.hass, config, sensors.extend)
...
class ClientTests(unittest.TestCase):
    def test_tcp_timeout_and_cleanup(self):
        with patch("custom_components.kwb.client.socket.create_connection") as connect:
            validate_connection({"type": "tcp", "host": "boiler", "port": 23})
        connect.assert_called_once_with(("boiler", 23), timeout=10)
        connect.return_value.settimeout.assert_called_once_with(None)
        connect.return_value.close.assert_called()

    def test_stop_unblocks_reader(self):
        reading = Event()
        closed = Event()
        ...
        with patch("custom_components.kwb.client.socket.create_connection") as connect:
            connect.return_value.recv.side_effect = recv
            connect.return_value.shutdown.side_effect = lambda how: closed.set()
